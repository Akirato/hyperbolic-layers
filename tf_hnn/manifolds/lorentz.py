...
from ..utils.functional import cosh, acosh_

class Lorentz(Manifold):
    """
    This class can be used for mathematical functions on the Lorentz model.
    The Lorentz model with curvature -c< 0 is the set of points n-dim Euclidean space such that |x|^2 < 1/c
    ...
    def mobius_matvec(self, m, x):
        ...
        res_c = (
            cosh(mx_norm / x_norm * acosh_(self._sqrt_c * x_norm)) * mx / (mx_norm * self._sqrt_c)
        )
        cond = tf.reduce_prod(
    ...
    def expmap(self, u, p):
        ...
        second_term = (
            cosh(self._sqrt_c / 2 * self.lambda_x(p) * u_norm) * u / (self._sqrt_c * u_norm)
        )
    ...
    def expmap0(self, u):
        ...
        u_norm = self.clipped_norm(u)
        gamma_1 = tf.math.cosh(self._sqrt_c * u_norm) * u / (self._sqrt_c * u_norm)
        return gamma_1
    ...
    def logmap0(self, p):
        ...
        scale = 1.0 / self._sqrt_c * arcosh(self._sqrt_c * p_norm) / p_norm
        return scale * p
    ...
    def logmap(self, x, y):
        ...
        return 2 * (x_y / norm_x_y) * tf.math.arcosh(tanh) / (sqrt_k * lambda_x)
    ...
    def dist(self, x, y):
        ...
        return 2./self._sqrt_c * acosh_( self._sqrt_c * norm)
